...
import { Menu, SquareLibrary, User, X } from "lucide-react";
const MusicNavbar = ({
  songsList,
  setSongsList,
  currentSong,
      ...
      <nav className="font-grenzeGotisch fixed top-0 left-0 w-full bg-[#1c1917] z-50 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <Link
              to="/"
              className="flex items-center text-[#e11d48] font-bold text-xl"
            >
            ...
            <div className="hidden md:flex items-center space-x-6">
              <Link
                to="/favorites"
                className="text-gray-700 hover:text-[#e11d48]"
              >
                <SquareLibrary className="w-7 h-7" />
              </Link>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-transparent shadow-lg px-4 py-4 space-y-4">
            <div className="flex  justify-around">
              <Link
                to="/favorites"
                className=" flex text-gray-700 hover:text-[#e11d48] text-base"
                onClick={() => setMobileMenuOpen(false)}
              >
                <SquareLibrary className=" mr-2" /> Favorites
              </Link>
              <Link
                to="/user"
                className="flex text-gray-700 hover:text-[#e11d48] text-base"
